<?php
  ...
  $response = BtcpayserverPayment::processPayment(true);
  $status = $response[0];
  $message = @$response[1];

  $tx = Params::getParam('btcpayserver_transaction_id');
  ...
  if(OSP_DEBUG) {
    ...
    mail(osc_contact_email() , 'OSCLASS PAY - BTCPAYSERVER RETURN DEBUG RESPONSE', $emailtext);
  }

  // $data = osp_get_custom(Params::getParam('extra'));
  // $product_type = explode('x', @$data['product']);
  $product_type = Params::getParam('btcpayserver_product_type');
